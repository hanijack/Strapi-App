   ...
   <div className="flex flex-col p-6 md:p-8 xl:px-12 2xl:px-16">
    <div>
      ...
      <div className="flex flex-col md:flex-row  ">{data.map(( item) =>(
        <Link key={item.id} href={`./projects/${item.title.split(" ").join("").toLowerCase()}`} className="cursor-pointer flex flex-col gap-4 bg-slate-300 relative w-full h-[400px] md:w-1/2 xl:w-1/3 2xl:w-1/4 shadow-lg hover:opacity-95 transition-all ease-in-out duration-200 ">
